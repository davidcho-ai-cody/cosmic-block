...
public static class Sprint3PlayProbe {
 ...
 public static void Run(){Sprint3Builder.Validate();EditorSceneManager.OpenScene(Sprint3Builder.ScenePath);var s=UnityEngine.Object.FindFirstObjectByType<GameSession>();var so=new SerializedObject(s);so.FindProperty("bestScoreKey").stringValue=Key;so.ApplyModifiedPropertiesWithoutUndo();had=PlayerPrefs.HasKey(Key);old=PlayerPrefs.GetInt(Key,0);PlayerPrefs.SetInt(Key,8520);PlayerPrefs.Save();started=EditorApplication.timeSinceStartup;frames=stage=0;Application.logMessageReceived+=Log;EditorApplication.update+=Check;EditorApplication.EnterPlaymode();}
